...
class Gestures {
    ...
    static async checkIfDisplayedWithSwipeUp (element:WebdriverIO.Element, maxScrolls:number, amount = 0){
        ...
        if (!await element.isDisplayed() && amount <= maxScrolls) {
            ...
            await this.positionElementToCenter(element);
        }
    }

    static async positionElementToCenter(element:WebdriverIO.Element) {
        const editionBoxSize = 450;
        if(await element.isDisplayed()) {
            const {y: elementY} = await element.getLocation();
            // const elementHeight = await element.getSize('height');
            const {height, width} = await browser._getWindowSize();
            const startX = width /2;
            const endX = startX;
            const startY = elementY;
            const endY = height/2 - editionBoxSize;
            await this.swipe(
                {x: startX, y: startY},
                {x: endX, y: endY},
            );
    ...
    static async longSwipeRight (percentage = 1) {
        // await this.swipeOnPercentage(
        //     this.calculateXY(SWIPE_DIRECTION.right.start, percentage),
        //     this.calculateXY(SWIPE_DIRECTION.right.end, percentage),
        // );
